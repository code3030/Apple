// function () {} -> 호출 (function () {}) ()
// () => {} -> 호출 (() => {}) ()
// 전역변수 배열을 피할려고 사용
// const arr = [1,2,3]; -> 자바스크립트에서는 전역변수를 선언하는건 바람직하지 않다.
// (() => {
//      const arr = [1,2,3]; // 지역변수
// })();
// dlkafj;lkdfja;ldfkja;l
(() => {
    ...
    const sceneInfo = [ // 각 scene에 대한 정보를 객체에 담는다.
        ...
        { 
            // 첫번째 객체 0
            type: 'sticky', // type을 정한다. (sticky와 normal)
            heightNum: 5, // 브라우저 높이의 5배로 scrollHeight 세팅 (고정값이 아닌 창 사이즈 변경에도 대응하기 위해 따로 함수 처리)
        ...
        { 
            ...
            values: {
                ...
                imageSequenece: [0, 959],
                canvas_opacity_in: [0, 1, { start: 0, end: 0.1 }], // 등장시 부드럽게 
                canvas_opacity_out: [1, 0, { start: 0.95, end: 1 }], // 사라질때 부드럽게 
    ...
    function playAnimation () {
        ...
        switch (currentScene) {
            case 0:
                ...
                objs.canvas.style.opacity = calcValues(values.canvas_opacity, currentYOffset); // 캔버스 opacity 실행

                // console.log('0 play');
                // 텍스트 opacity
                // translate3d 하드웨어 가속이 보장 (퍼포먼스가 좋다) => 3d가 아니더라도 사용한다. 
                if (scrollRatio <= 0.22) {
                    // in
            ...
            case 2:
                ...
                // 변수가 겹쳐서 보호를 해주어야 한다. => 90% 구간쯤 도착했을 시 그려준다. => {} 괄호 안에 있는 변수는 {}에서의 지역 변수다.
                if (scrollRatio > 0.7) {
            ...
            case 3:
                ...
                // 문제점: 스크롤바가 영역을 차지 한다. innerWidth는 스크롤바를 포함한 영역이다. => recalculatedInnerWidth 값을 구할때, document.body.offsetHeight 사용한다.
                if (!values.rectStartY) { 
                    // values.rectStartY = objs.canvas.getBoundingClientRect().top;
                    // 문서의 제일 상단을 기준 // 기준점을 바꿀 수 있다. => CSS로 속성값을 바꾼다.(static이 아닌 값)
                ...
                if (scrollRatio < values.rect1X[2].end) { // 애니메이션 end시점 이전이라면 
                    ...
                    // 이미지 블렌드
                    // imageBlendY: [ 0, 0, { start: 0, end: 0 } ] blend-image-2.jpg
                    values.blendHeight[0] = 0;
                    values.blendHeight[1] = objs.canvas.height;
                    values.blendHeight[2].start = values.rect1X[2].end;
                    // 속도를 조절(end작으면 빠르고, 크면 느리다.) => 전체가 1
                    // start시점(0%)부터 20% 
                    ...
                    objs.context.drawImage(objs.images[1],
                        0, objs.canvas.height - blendHeight, objs.canvas.width, blendHeight,// 소스 이미지에서 가져오는 부분
                        0, objs.canvas.height - blendHeight, objs.canvas.width, blendHeight) // 실제 캠버스에 그리는 부분 => 이미지와 캠버스 크기가 똑같아서 계산해줄게 없다.(다를 시 비율계산)

                    objs.canvas.classList.add('sticky');
                    objs.canvas.style.top = `${-(objs.canvas.height - objs.canvas.height * canvasScaleRatio) / 2}px`; // scale 이전

                    // 스케일
                    if (scrollRatio > values.blendHeight[2].end) {
                        values.canvas_scale[0] = canvasScaleRatio;
                        values.canvas_scale[1] = document.body.offsetWidth / (1.5 * objs.canvas.width); // 브라우저의 폭에 반응(내맘대로), 분모에 1.5를 곱해 값을 작게 만들어준다.
                        // console.log(values.canvas_scale[1])
                        values.canvas_scale[2].start = values.blendHeight[2].end;
                        values.canvas_scale[2].end = values.canvas_scale[2].start + 0.2; // start 후에 씬에 해당하는 20%동안 재생 => 이미지블렌드 후 축소되는 구간까지 전체 스크롤 구간의 총 40%
    ...
    function scrollLoop() {
        ...
        playAnimation();
        // document.body.setAttribute('id', `show-scene-${currentScene}`); // body에 씬에 맞게 id 부여, 변수와 문자열 조합이라 템플릿 언어사용
    }
